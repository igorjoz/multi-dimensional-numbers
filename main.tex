...
\begin{frame}{Dlaczego w ogóle ``liczby wielowymiarowe''?}
  \begin{itemize}
    ...
    \item Potem przejdziemy do 4 wymiarów (kwaterniony) i 8 wymiarów (oktoniony).
  \end{itemize}
...
\begin{frame}{Definicja liczb zespolonych}
  \begin{columns}[T]
    ...
    \begin{column}{0.5\textwidth}
      \centering
      \includesvg[width=0.75\textwidth]{Complex_number_illustration.svg}\\[5pt]
...
\begin{frame}{Zastosowania liczba zespolonych: Przetwarzanie sygnałów}
  \begin{itemize}
    \item Liczby zespolone są fundamentalne w analizie sygnałów (dźwięk, obraz, radio).
    \item Wzór Eulera $e^{i\omega t} = \cos(\omega t) + i\sin(\omega t)$ pozwala reprezentować sygnał jako sumę fal o danej amplitudzie i fazie.
  \end{itemize}
  \begin{center}
    \includesvg[width=0.4\textwidth]{images/FFT_of_Cosine_Summation_Function.svg}
    \\
    {\tinyit{Źródło: \url{https://commons.wikimedia.org/wiki/File:FFT_of_Cosine_Summation_Function.svg}}}
  \end{center}
\end{frame}

\begin{frame}{Zastosowania liczb zespolonych: Sieci neuronowe}
    \begin{itemize}
        \item \textbf{Zespolone sieci neuronowe} (Complex-Valued Neural Networks, CVNN) to rozszerzenie klasycznych sieci, gdzie wagi, biasy i aktywacje są liczbami zespolonymi.
        \item \textbf{Zalety:}
        \begin{itemize}
            \item Naturalnie przetwarzają dane zespolone, np. z radaru, sonaru, MRI, zachowując informację o fazie.
            \item Pojedynczy neuron zespolony może być bardziej wyrazisty niż dwa neurony rzeczywiste.
            \item Lepsze zdolności do generalizacji w problemach z falami i oscylacjami.
        \end{itemize}
    \end{itemize}
\end{frame}
...
\begin{frame}{Kwaterniony -- definicja}
  Pytanie Hamiltona: \\
...
\begin{frame}{Dowód: mnożenie kwaternionów nie jest przemienne}
  \begin{block}{Teza}
    W $\mathbb{H}$ zachodzi $ij \neq ji$.
  \end{block}

  \begin{proof}
    Z definicji jednostek $i^2=j^2=k^2=ijk=-1$ mamy $ij=k$ oraz $ji=-k$, zatem $ij\neq ji$. \qedhere
  \end{proof}
...
\begin{frame}{Oktoniony w skrócie}
  \begin{itemize}
  ...
  Zastosowania:
  \begin{itemize}
    \item teoretyczna fizyka wysokich energii, symetrie wyjątkowe,
...
\begin{frame}{Różnice w działaniach}
  \begin{itemize}
    ...
    ale jest łączne.
    \item W $\mathbb{O}$: nawet łączność upada, tzn.
    $(ab)c \neq a(bc)$ w ogólności.
  \end{itemize}
  ...
  ale płacimy za to utratą przydatnych własności algebraicznych.
\end{frame}
...
\begin{frame}{Źródła}
  \scriptsize
  \begin{itemize}
    \item Wikipedia, \emph{Quaternion}, \url{https://en.wikipedia.org/wiki/Quaternion}
    \item Wikipedia, \emph{Quaternions and spatial rotation}, \url{https://en.wikipedia.org/wiki/Quaternions_and_spatial_rotation}
    \item Wikipedia, \emph{Gimbal lock}, \url{https://en.wikipedia.org/wiki/Gimbal_lock}
    \item Wikipedia, \emph{Octonion}, \url{https://en.wikipedia.org/wiki/Octonion}
    \item Wikipedia, \emph{Cayley--Dickson construction}, \url{https://en.wikipedia.org/wiki/Cayley-Dickson_construction}
    \item 3Blue1Brown, \emph{Quaternions and 3d rotation, explained interactively}, \url{https://youtu.be/zjMuIxRvygQ}
    \item 3Blue1Brown, \emph{Visualizing the 4d numbers Quaternions}, \url{https://youtu.be/d4EgbgTm0Bg}
    \item Dokumentacja Unity i Blender: reprezentacja rotacji przez kwaterniony.
\end{itemize}
