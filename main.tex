...
od liczb zespolonych do kwaternionów\\
(+ oktoniony jako ciekawostka)}
...
\begin{frame}{Dlaczego w ogóle ``liczby wielowymiarowe''?}
  ...
  \vspace{0.3cm}
  Idea: \textbf{rozszerzamy pojęcie liczby}, żeby mieć lepsze modele obrotów, fal, sygnałów, ruchu 3D, itp.
\end{frame}

\begin{frame}{Krótka historia}
  \begin{itemize}
    \item XVIII w.: Euler, Gauss -- porządne zdefiniowanie liczb zespolonych.
  ...
  \vspace{0.4cm}
  W XXI w.: kwaterniony są standardowo używane w grafice 3D (Unity, Blender), dronach i satelitach.
\end{frame}
...
\begin{frame}{Tabela mnożenia}
  ...
  \vspace{0.2cm}
  To jest dokładnie typ slajdu, który prowadzący chciał: definicja + przykład rachunkowy.
\end{frame}
...
\begin{frame}{Kwaterniony jako macierze $2\times 2$ zespolone}
  ...
  \vspace{0.3cm}
  Ten slajd spełnia wymaganie prowadzącego: ``kwaterniony jako macierze $2\times 2$
  liczb zespolonych''.
\end{frame}
...
\begin{frame}{Źródła}
  ...
  \vspace{0.3cm}
  \tinyit{Prezentacja przygotowana w \LaTeX{} beamer, zgodnie z wymaganiami seminarium
           (definicje, przykłady rachunkowe, jedno rozumowanie, zastosowania, tło historyczne).}
\end{frame}
